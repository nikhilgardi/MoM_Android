...
public void addListenerOnSpinnerItemSelection() {
	 ...
	 public String[] getAllOperators() {
	        ...
	        try {
	            ...
	            HttpPost httppost = new HttpPost("http://180.179.67.72/nokiaservice/getProductByProductType.aspx?OperatorType=2"); 
	            HttpResponse response = httpclient.execute(httppost);
				HttpEntity entity = response.getEntity();
	 ...
	 private String Get_OperatorID(String strOperatorName) {
		 ...
		 HttpClient httpclient = new DefaultHttpClient();
	        try {
	        	...
	        	HttpPost httppostcheck = new HttpPost("http://180.179.67.72/nokiaservice/getOperatorIdByOperatorName.aspx?OperatorName=" + urlEncode(strOperatorName.toString()));
	...
	private int validate() {
		...
		if (operatorSpinner.getSelectedItemPosition() < 1) {
			return 1;
		} else if (numField_SubscriberId.getText().toString().length() < 7) {
			return 2;
