...
public class HomeActivity1 extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		reintent = new Intent(HomeActivity1.this, MainActivity1.class);
	}
	...
	private void init() {
		...
		if ((pref.getString("user_sessionMOM", "test").equals("MOM"))
				|| (pref.getString("user_sessionMOM", "test").equals("B2C")))
...

		{
			...
			try {
				String[] strOperators = new String[] {
						getResources().getString(R.string.prompt_spinner_select_operator), "AIRTEL DIGITAL", "BIG TV",
						"DISH", "SUN DIRECT", "TATA SKY", "VIDEOCON DTH" };
	...
	private int validate() {
		...
		if ((Operatorid.equals("21")) || (Operatorid.equals("24"))
				|| (Operatorid.equals("ADG")) || (Operatorid.equals("TSK"))) {
		...
		} else if ((Operatorid.equals("7")) || (Operatorid.equals("14"))
				|| (Operatorid.equals("DSH")) || (Operatorid.equals("SUN"))) {
			if (operatorSpinner.getSelectedItemPosition() < 1) {
				return 1;
	...
	private void newrechargecheck() {
		...
		// //////// Toast.makeText(HomeActivity1.this, this.newoutputrecharge,
	...
	private void rechargePost() {
		...
		try {
			...
			myHelpez.SetMyRechargeMobileNumber(numField_SubscriberId.getText()
					.toString());
			myHelpez.SetMyRechargeAmount(amountField.getText().toString());
			myHelpez.SetCustomerMobileNumber(CustomerNumber.getText()
					.toString());
...

		{
			try {
				...
				// HttpPost("http://msvc.money-on-mobile.net/WebServiceV3Trans.asmx/DoDTHRecharge");
				HttpPost httppost = new HttpPost(
						"http://msvc.money-on-mobile.net/WebServiceV3Trans.asmx/DoDTHRechargeV2");
				...
				nameValuePairs.add(new BasicNameValuePair("CompanyID", myHelpez
						.GetMyCompanyId()));
				nameValuePairs.add(new BasicNameValuePair("CustomerNumber",
						myHelpez.GetCustomerMobileNumber()));
				final HttpParams httpParams = httpclient.getParams();
				...
				Log.i("postData",
						Get_OperatorID(myHelpez.GetMyRechargeOperator())
								.toString());
				Log.i("CompanyIDDTH", myHelpez.GetMyCompanyId());
				/*
...

		{
			...
			try {
				...
				// InputStream in = new
				// ByteArrayInputStream(responseBody.getBytes("UTF-8"));
				// new NewXmlPullParsing(in);
		...
		void parseTag(int event) {
			...
			case XmlPullParser.TEXT:
				...
				case 101:
					...
					// Toast.LENGTH_SHORT).show();
					// //////// Toast.makeText(HomeActivity1.this,
	...
	public class NewXmlPullParsing {
		...
		void parseTag(int event) {
			...
			case XmlPullParser.TEXT:
				...
				// /////// Toast.makeText(HomeActivity1.this, newoutputrecharge,
				// Toast.LENGTH_LONG).show();
	...
	public void onClick(View view) {
		if (validate() == 0) {
			...
			// AlertDialog.Builder(HomeActivity1.this);
			...
			alertDialog.setMessage(getResources().getString(R.string.Lbl_SubscriberID) + " "
					+ numField_SubscriberId.getText().toString() + "\n"
			...
			case 2:
				secondback_responseText.setVisibility(View.VISIBLE);
				secondback_responseText.setText(getResources().getString(R.string.prompt_Validity_subscriberId));
				numField_SubscriberId.setText("");
				break;
